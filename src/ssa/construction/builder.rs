...
pub struct BlockBuilder<'a> {
    ...
    current_mem_access_field_size: Option<ByteSize>,
}
...
impl<'a> BlockBuilder<'a> {
    ...
    pub fn set_mem_field_size(&mut self, size: usize) {
        if self.current_mem_access_field_size.is_none() {
            self.current_mem_access_field_size = Some(size.try_into().unwrap());
        }
    }

    /// returns (offset, size)
    pub fn take_mem_access(&mut self) -> (usize, ByteSize) {
        (self.take_mem_offset(), self.take_field_size())
    }

    fn take_mem_offset(&mut self) -> usize {
    ...
    fn take_field_size(&mut self) -> ByteSize {
        self.current_mem_access_field_size.take().unwrap()
    }
